...
public class Image_grid extends AppCompatActivity implements AdapterView.OnItemClickListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Random r = new Random();
        double targetSimi = 70 + (90 - 70) * r.nextDouble();
        int tarid = main.tarID;


        imgStruct tarimg = new imgStruct(tarid, targetSimi);
        pQueue.add(tarimg);
        for(i=0; i < imgList.size();i++){
            ...
            Glide.with(this).load(IMAGE_URL).asBitmap().into(new SimpleTarget<Bitmap>() {
                ...
                public void onResourceReady(Bitmap resource, GlideAnimation<? super Bitmap> glideAnimation) {
                    ...
                    Log.d("pQueue", "---------------------"+pQueue.size());
                    if(pQueue.size() == imgList.size() ){
                        List<image> list = new ArrayList<image>();

                        for(int j = 0; j < 9; j++){

                            Log.d("queue ", j+ "  "+pQueue.peek().getSimilarity());
                            imgStruct imgSort = pQueue.poll();
                            double similarity = imgSort.getSimilarity()/100;

                            long id = imgSort.getId();

                            image imagePre = DataSupport.find(image.class, id);
                        ...
                        for(int i=0;i<list.size();i++){
                            ...
                            Log.d("qqqqqq", "list    "+ i+"    " +list.get(i).getSimilarity());
